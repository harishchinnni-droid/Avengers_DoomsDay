...
config.LIVE_TRADING = True           # REAL-order path, against a fake broker
...
def full_opt(contract, symbol, opt_type, nfo):
    ...
    if k not in _cache:
        ...
        if _cache[k][0] is not None: BRK_DFS[contract.trading_symbol] = _cache[k][0]
    return _cache[k]
...
# ---------------- fake Angel broker (simulates SL-M on bar lows) ----------------
import live_orders
from live_orders import LiveFill
BRK = {"n": 0, "stops": {}, "dfs": BRK_DFS}
TICK = {}
def _df(tsym): return BRK["dfs"][tsym]
def _bar_start(): return NOW[0] - timedelta(seconds=5)
def _last_close(tsym):
    df = _df(tsym); df = df[df.index + INT <= NOW[0]]
    return float(df["close"].iloc[-1])
def _nid():
    BRK["n"] += 1; return f"F{BRK['n']}"
def enter_live(angel, tsym, token, qty):
    px = float(_df(tsym).loc[_bar_start(), "open"])
    return LiveFill(order_id=_nid(), filled_qty=qty, avg_price=px, status="complete")
def place_protective_stop(angel, tsym, token, qty, trig):
    i = _nid(); BRK["stops"][i] = dict(tsym=tsym, qty=qty, trig=trig, since=_bar_start(), live=True); return i
def update_protective_stop(angel, oid, tsym, token, qty, trig):
    BRK["stops"][oid]["live"] = False; return place_protective_stop(angel, tsym, token, qty, trig)
def resize_protective_stop(angel, oid, tsym, token, qty, trig):
    return update_protective_stop(angel, oid, tsym, token, qty, trig)
def order_status(angel, oid):
    st = BRK["stops"].get(oid)
    if not st or not st["live"]: return {"status": "open"}
    df = _df(st["tsym"]); df = df[(df.index >= st["since"]) & (df.index + INT <= NOW[0])]
    for ts, b in df.iterrows():
        o, lo, cl = float(b["open"]), float(b["low"]), float(b["close"])
        if o > 0 and (o - lo) / o > config.MAX_INTRABAR_COLLAPSE: lo = min(o, cl)
        if lo <= st["trig"] + 1e-6:
            st["live"] = False
            px = max(min(o, st["trig"]), lo)
            return {"status": "complete", "orderid": oid, "filledshares": st["qty"], "averageprice": px}
    return {"status": "open"}
def _parse_fill(row):
    return LiveFill(order_id=row["orderid"], filled_qty=int(row["filledshares"]),
                    avg_price=float(row["averageprice"]), status="complete")
def exit_live(angel, tsym, token, qty, stop_id, reason):
    if stop_id and stop_id in BRK["stops"]: BRK["stops"][stop_id]["live"] = False
    px = _last_close(tsym)
    if reason.startswith("Target") and tsym in TICK:
        pos, ltp = TICK[tsym]
        k = int(reason.split()[1]) - 1
        px = min(ltp, pos.targets[k])
    return LiveFill(order_id=_nid(), filled_qty=qty, avg_price=px, status="complete")
for n in ["enter_live", "place_protective_stop", "update_protective_stop", "resize_protective_stop",
          "order_status", "_parse_fill", "exit_live"]:
    setattr(live_orders, n, globals()[n])
live_orders.kill_switch_active = lambda: False
_orig_tick = oe._evaluate_live_tick
def _tick(angel, pos, when, ltp, sq):
    TICK[pos.contract.trading_symbol] = (pos, ltp); return _orig_tick(angel, pos, when, ltp, sq)
oe._evaluate_live_tick = _tick
oe._retry_protective_stop = lambda angel, pos: None
...
def norm(o):
    o = o[cols].copy()
    o["Exit Reason"] = o["Exit Reason"].astype(str).map(lambda r: "STOP (any)" if ("Stop" in r and "tightened" not in r) else r)
    o["Net P/L (Rs)"] = o["Net P/L (Rs)"].astype(float).round(2)
